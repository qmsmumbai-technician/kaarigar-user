...
- **Accept** → opens a Razorpay Payment Link (manually created in Razorpay Dashboard, no code) in a new tab
...
**⚠️ Manual step required (not automated yet):** there's no Cloud Function/webhook confirming Razorpay payments, so after a technician pays, **you must manually add/update the `paidUntil` field** on their document in the Firebase console (Firestore → technicians → their doc → set `paidUntil` to a timestamp 3 months out). Automating this fully would need a Cloud Function listening to Razorpay's webhook — a real backend piece, deferred for now given expected early-stage volume is small. Revisit once subscriber count makes manual verification painful.

**Setup still needed:** create a Razorpay Payment Link (₹300, "Kaarigar 3-month subscription") and paste its URL into the `PAYMENT_LINK_URL` constant near the top of the Technician app's script — currently a placeholder.
...
- No in-app payments beyond the manual Razorpay Payment Link flow — ratings are currently static placeholder values, no booking/scheduling — by design, matches the original "directory + call" scope
...
- [ ] Create the Razorpay Payment Link and paste it into `PAYMENT_LINK_URL` (Technician app)
...
- [ ] Automate payment confirmation via Cloud Function + Razorpay webhook once manual `paidUntil` updates become too frequent to do by hand
